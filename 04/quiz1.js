...
const photoCols = 3;
const totalPhotoIndex = Math.ceil(todayPhoto.length / photoCols);
let photoIndex = 1;
const $wrap = document.getElementById('wrap');
const $btnL = document.getElementById('buttonLeft');
const $btnR = document.getElementById('buttonRight');
...
function showImg() {
  ...
  for (let i = 0; i < photoCols; i++) {
    imgNum = (photoIndex - 1) * photoCols + i;
    if (imgNum < todayPhoto.length) {
      // str += '<img src="' + todayPhoto[imgNum].img + '"> ';
      str += `<img src="${todayPhoto[imgNum].img}">`;
    }
  }
  $wrap.innerHTML = str;
  $page.innerHTML = `${photoIndex} / ${totalPhotoIndex}`;
}
